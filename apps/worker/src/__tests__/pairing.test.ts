...
describe('pairing algorithm', () => {
  ...
  describe('replaying a draw', () => {
    const now = Date.now();

    it('pairs the two who drew, over a better-scoring open pair', () => {
      // `c` and `d` are the pair the scorer would otherwise reach for: same
      // tier, different cities, waiting longest. The pin has to beat them.
      const entries = [
        {
          userId: 'a',
          joinedAt: now - 1000,
        ...
        { userId: 'c', joinedAt: now - 60000, city: 'Lahore', skillTier: 3, roundNumber: 1 },
        { userId: 'd', joinedAt: now - 60000, city: 'Karachi', skillTier: 3, roundNumber: 1 },
      ];
      const pair = findBestPair(entries, now);
      expect([pair?.candidate.userId, pair?.partner.userId].sort()).toEqual(['a', 'b']);
    });

    it('will not hand a pinned player to anyone else', () => {
      // The opponent has not come back yet. `a` waits rather than being paired
      // off with the stranger sitting next to them in the queue.
      const entries = [
        {
          userId: 'a',
          joinedAt: now - 60000,
        ...
        { userId: 'c', joinedAt: now - 60000, city: 'Karachi', skillTier: 3, roundNumber: 1 },
      ];
      expect(findBestPair(entries, now)).toBeNull();
    });

    it('refuses a pin only one side agrees to', () => {
      const entries = [
        {
          userId: 'a',
          joinedAt: now - 60000,
        ...
        { userId: 'b', joinedAt: now - 60000, city: 'Karachi', skillTier: 3, roundNumber: 1 },
      ];
      expect(findBestPair(entries, now)).toBeNull();
    });

    it('keeps the round rule — a pin cannot reach across rounds', () => {
      const entries = [
        {
          userId: 'a',
          joinedAt: now - 60000,
    ...
    it('leaves unpinned players free to pair with each other', () => {
      const entries = [
        {
          userId: 'a',
          joinedAt: now - 60000,
        ...
        { userId: 'c', joinedAt: now - 60000, city: 'Karachi', skillTier: 3, roundNumber: 1 },
        { userId: 'd', joinedAt: now - 60000, city: 'Lahore', skillTier: 3, roundNumber: 1 },
      ];
      const pair = findBestPair(entries, now);
      expect([pair?.candidate.userId, pair?.partner.userId].sort()).toEqual(['c', 'd']);
    });
  });
});
